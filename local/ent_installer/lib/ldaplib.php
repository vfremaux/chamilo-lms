...
function ldap_supported_usertypes() {
    ...
    $types['default'] = get_string('default', 'local_ent_installer');
    return $types;
}
...
function ldap_isgroupmember($ldapconnection, $userid, $group_dns, $member_attrib) {
    ...
    foreach ($group_dns as $group) {
        ...
        if (!empty($search) && ldap_count_entries($ldapconnection, $search)) {
            $info = ldap_get_entries_chamilo($ldapconnection, $search);
            if (count($info) > 0 ) {
 ...
 */
function ldap_connect_chamilo($host_url, $ldap_version, $user_type, $bind_dn, $bind_pw, $opt_deref, &$debuginfo) {
 ...
 */
function ldap_get_entries_chamilo($ldapconnection, $searchresult) {
