...
<<<<<<< HEAD
                            ...
                            Display :: display_confirmation_message(
                                sprintf(get_lang('XResultsCleaned'), $quantity_results_deleted)
                            );
=======
                            $quantity_results_deleted = $objExerciseTmp->clean_results(true);
                            Display :: display_confirmation_message(sprintf(get_lang('XResultsCleaned'), $quantity_results_deleted));
...
<<<<<<< HEAD
    echo '<a href="exercise_admin.php?'.api_get_cidreq().'">'.Display :: return_icon(
        'new_exercice.png',
        get_lang('NewEx'),
        '',
        ICON_SIZE_MEDIUM
    ).'</a>';
    echo '<a href="question_create.php?'.api_get_cidreq().'">'.Display :: return_icon(
        'new_question.png',
        get_lang('AddQ'),
        '',
        ICON_SIZE_MEDIUM
...
<<<<<<< HEAD
    echo '<a href="hotpotatoes.php?'.api_get_cidreq().'">'.Display :: return_icon(
        ...
        get_lang('ImportHotPotatoesQuiz'),
        '',
        ICON_SIZE_MEDIUM
    ).'</a>';
    // link to import qti2 ...
    echo '<a href="qti2.php?'.api_get_cidreq().'">'.Display :: return_icon(
        'import_qti2.png',
        get_lang('ImportQtiQuiz'),
        '',
        ICON_SIZE_MEDIUM
    ).'</a>';
    echo '<a href="upload_exercise.php?'.api_get_cidreq().'">'.Display :: return_icon(
        'import_excel.png',
        get_lang('ImportExcelQuiz'),
        '',
        ICON_SIZE_MEDIUM
    ).'</a>';
=======
...
if ($total > $limit) {
    ...
    if ($page) {
        echo "<a href=\"".api_get_self()."?".api_get_cidreq()."&amp;page=".($page - 1)."\">".Display :: return_icon(
            'action_prev.png',
...
<<<<<<< HEAD
                    ...
                    if (!empty($count_exercise_not_validated)) {
                        $results_text = $count_exercise_not_validated == 1 ? get_lang('ResultNotRevised') : get_lang(
                            'ResultsNotRevised'
                        );
                    ...
                    if ($session_id == $row['session_id']) {
                        //Settings
                        $actions = Display::url(Display::return_icon('edit.png', get_lang('Edit'), '', ICON_SIZE_SMALL), 'admin.php?'.api_get_cidreq().'&exerciseId='.$my_exercise_id);
                        ...
                        if ($autolaunch_setting_on) {
                            ...
                            if ($row['autolaunch'] == 0) {
                                $icon = Display::return_icon('launch_na.png', get_lang('AutoLaunch'));
                            }
                            $actions .= Display::url($icon, 'exercice.php?'.api_get_cidreq().'&choice=set_autolaunch&sec_token='.$token.'&exerciseId='.$my_exercise_id);
                        }
...
<<<<<<< HEAD
                        ...
                        if ($exercise_obj->exercise_was_added_in_lp == true) {
                            ...
                            if ($row['active'] == 0 || $visibility == 0) {
                                $actions .= Display::url(Display::return_icon('invisible.png', get_lang('Activate'), '', ICON_SIZE_SMALL), 'exercice.php?'.api_get_cidreq().'&choice=enable&sec_token='.$token.'&page='.$page.'&exerciseId='.$my_exercise_id);
                            } else {
                                // else if not active
                                $actions .= Display::url(Display::return_icon('visible.png', get_lang('Deactivate'), '', ICON_SIZE_SMALL), 'exercice.php?'.api_get_cidreq().'&choice=disable&sec_token='.$token.'&page='.$page.'&exerciseId='.$my_exercise_id);
                            }
                        }
                        // Export qti ...
                        $actions .= Display::url(Display::return_icon('export_qti2.png', 'IMS/QTI', '', ICON_SIZE_SMALL), 'exercice.php?choice=exportqti2&exerciseId='.$my_exercise_id);
                    } else {
                        // not session
                        $actions = Display::return_icon('edit_na.png', get_lang('ExerciseEditionNotAvailableInSession'));
                        $actions .='<a href="exercise_report.php?'.api_get_cidreq().'&exerciseId='.$my_exercise_id.'">'.Display :: return_icon('test_results.png', get_lang('Results'), '', ICON_SIZE_SMALL).'</a>';
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
            ...
            $actions .= '<a href="hotpotatoes_exercise_report.php?'.api_get_cidreq(
            ).'&path='.$path.'">'.Display :: return_icon(
                'test_results.png',
                get_lang('Results'),
                '',
                ICON_SIZE_SMALL
            ...
            if ($active) {
                ...
                $actions .= '      <a href="'.$exercicePath.'?'.api_get_cidreq(
                ).'&hpchoice=disable&amp;page='.$page.'&amp;file='.$path.'">'.Display::return_icon(
                    'visible.png',
                    get_lang('Deactivate'),
                    '',
                    ICON_SIZE_SMALL
                ...
                $actions .= '    <a href="'.$exercicePath.'?'.api_get_cidreq(
                ).'&hpchoice=enable&amp;page='.$page.'&amp;file='.$path.'">'.Display::return_icon(
                    'invisible.png',
                    get_lang('Activate'),
                    '',
                    ICON_SIZE_SMALL
            ...
            $actions .= '<a href="'.$exercicePath.'?'.api_get_cidreq(
            ).'&amp;hpchoice=delete&amp;file='.$path.'" onclick="javascript:if(!confirm(\''.addslashes(
...
<<<<<<< HEAD
                $item .= Display::tag(
                    'td',
                    '<a href="showinframes.php?'.api_get_cidreq().'&file='.$path.'&cid='.api_get_course_id(
                    ).'&uid='.api_get_user_id().'"'.(!$active ? 'class="invisible"' : '').'">'.$title.'</a>'
                );
=======
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
                //$item .= Display::tag('td', '');
                $actions = '<a href="hotpotatoes_exercise_report.php?'.api_get_cidreq(
                ).'&path='.$path.'&filter_by_user='.api_get_user_id().'">'.Display :: return_icon(
                    'test_results.png',
                    get_lang('Results'),
                    '',
                    ICON_SIZE_SMALL
