            ...
            switch ($this->type) {
                case 3: //aicc
                    ...
                    if (is_object($oItem)) {
                        ...
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - learnpath::__construct() - aicc object with id '.$my_item_id.' set in items[]',
                                0
                            );
                        }
                    }
                ...
                default:
                    ...
                    if (is_object($oItem)) {
                        ...
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - learnpath::__construct() '.__LINE__.' - object with id '.$my_item_id.' set in items[]',
                                0
                            );
                        }
                    }
...
<<<<<<< HEAD
            ...
            if ($row['parent_item_id'] == 0 || empty ($this->items[$row['parent_item_id']])) {
                ...
                if (is_object($this->items[$row['parent_item_id']])) {
                    ...
                    if ($this->debug > 2) {
                        error_log(
                            'New LP - learnpath::__construct() '.__LINE__.' - The parent item ('.$row['parent_item_id'].') of item '.$row['id'].' could not be found',
                            0
                        );
                    }
                }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if ($this->debug > 2) {
            error_log(
                'New LP - learnpath::__construct() '.__LINE__.' - End of learnpath constructor for learnpath '.$this->get_id(
                ),
                0
            );
        }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        $description = Database::escape_string(
            api_htmlentities($description, ENT_QUOTES, $charset)
        ...
        switch ($origin) {
            ...
            default:
                ...
                $sql_insert = "INSERT INTO $tbl_lp (c_id, lp_type,name,description,path,default_view_mod, default_encoding,display_order,content_maker,content_local,js_lib,session_id, created_on, publicated_on, expired_on, category_id) ".
                    "VALUES ($course_id, $type,'$name','$description','','embedded','UTF-8','$dsp','Chamilo','local','','".$session_id."', '".api_get_utc_datetime(
            ...
            if ($current_item->is_done() || $current_status == 'browsed' || $current_status == 'failed') {
                ...
                foreach ($parent->get_children() as $child) {
                    ...
                    if ($child != $item) {
                        if ($debug) {
                            error_log(
                                'Looking at brother with ID '.$child.', status is '.$this->items[$child]->get_status(),
                                0
                            );
                        }
                        //if($this->items[$child]->status_is(array('completed','passed','succeeded')))
                        // Trying completing parents of failed and browsed items as well.
                        if ($this->items[$child]->status_is(
                            array(
                                'completed',
                                'passed',
                                'succeeded',
                            ...
                            if ($this->debug > 2) {
                                error_log(
                                    'Found one incomplete child of '.$parent_id.': '.$child.' is '.$this->items[$child]->get_status(
                                    ),
                                    0
                                );
                            }
        ...
        if ($this->type == 2 || $this->type == 3) {
            ...
            if (Database :: num_rows($res) > 0) {
                ...
                if (Database :: num_rows($res) > 0) { // Another learning path uses this directory, so don't delete it.
                    if ($this->debug > 2) {
                        error_log(
                            'New LP - In learnpath::delete(), found other LP using path '.$path.', keeping directory',
                            0
                        );
                    }
                } else {
                    ...
                    $course_scorm_dir = api_get_path(
                        SYS_COURSE_PATH
                    ).$course_rel_dir; // The absolute system path for this course.
                    if ($delete == 'remove' && is_dir($course_scorm_dir.$path) and !empty ($course_scorm_dir)) {
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - In learnpath::delete(), found SCORM, deleting directory: '.$course_scorm_dir.$path,
                                0
                            );
                        }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if (is_array($audio) && !empty ($audio['tmp_name']) && $audio['error'] === 0) {
            ...
            if ($pi['extension'] == 'mp3') {
                ...
                $path = FileManager::handle_uploaded_document(
                    $c_det,
                    $audio,
                    $bp,
                    '/audio',
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function edit_item_prereq($id, $prerequisite_id, $mastery_score = 0, $max_score = 100)
    {
        ...
        if ($this->debug > 0) {
            error_log(
                'New LP - In learnpath::edit_item_prereq('.$id.','.$prerequisite_id.','.$mastery_score.','.$max_score.')',
                0
            );
        }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function first()
    {
        ...
        if (!empty($this->last_item_seen) &&
            ...
            if ($this->debug > 2) {
                error_log(
                    'New LP - In learnpath::first() - Last item seen is '.$this->last_item_seen.' of type '.$this->items[$this->last_item_seen]->get_type(
                    ),
                    0
                );
            }
            ...
            if ($index == -1) {
                ...
                if ($this->debug > 2) {
                    error_log(
                        'New LP - Last item ('.$this->last_item_seen.') was found in items but not in ordered_items, panic!',
                        0
                    );
                }
            ...
            if ($this->debug > 2) {
                error_log(
                    'New LP - In learnpath::first() - No last item seen. New last = '.$this->last.'('.$this->ordered_items[$index].')',
                    0
                );
            }
        }
    ...
    public function get_navigation_bar()
    {
        ...
        if ($this->mode == 'fullscreen') {
            $navbar = '
                  <div class="buttons">
<<<<<<< HEAD
                    <a href="lp_controller.php?action=stats&'.api_get_cidreq(true).'&lp_id='.$lp_id.'" onClick="window.parent.API.save_asset();return true;" target="content_name_blank" title="stats" id="stats_link">
                    '.Display::return_icon('lp_stats.gif', get_lang('Reporting')).'</a>
                    <a href="" onClick="switch_item('.$mycurrentitemid.',\'previous\');return false;" title="previous">
                    '.Display::return_icon('lp_leftarrow.gif', get_lang('ScormPrevious')).'</a>
                    <a href="" onClick="switch_item('.$mycurrentitemid.',\'next\');return false;" title="next">
                    '.Display::return_icon('lp_rightarrow.gif', get_lang('ScormNext')).' </a>
                    <a href="lp_controller.php?action=mode&mode=embedded" target="_top" title="embedded mode">
                    '.Display::return_icon('view_choose.gif', get_lang('ScormExitFullScreen')).'</a>
                    </div>';
=======
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
            ...
            $navbar = '
                  <div class="buttons">
<<<<<<< HEAD
                    <a href="lp_controller.php?action=stats&'.api_get_cidreq(true).'&lp_id='.$lp_id.'" onClick="window.parent.API.save_asset();return true;" target="content_name" title="stats" id="stats_link">
                        '.Display::return_icon('lp_stats.gif', get_lang('Reporting')).'
                    </a>
                    <a href="" onClick="switch_item('.$mycurrentitemid.',\'previous\');return false;" title="previous">
                        '.Display::return_icon('lp_leftarrow.gif', get_lang('ScormPrevious')).'</a>
                    <a href="" onClick="switch_item('.$mycurrentitemid.',\'next\');return false;" title="next"  >
                    '.Display::return_icon('lp_rightarrow.gif', get_lang('ScormNext')).'
                    </a>
=======
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function get_next_index()
    {
        ...
        if ($this->debug > 2) {
            error_log(
                'New LP - Now looking at ordered_items['.($index).'] - type is '.$this->items[$this->ordered_items[$index]]->type,
                0
            );
        }
        while (!empty ($this->ordered_items[$index]) AND ($this->items[$this->ordered_items[$index]]->get_type(
        ) == 'dir' || $this->items[$this->ordered_items[$index]]->get_type(
        ) == 'dokeos_chapter') AND $index < $this->max_ordered_items) {
            $index++;
            if ($index == $this->max_ordered_items) {
                if ($this->items[$this->ordered_items[$index]]->get_type(
                ) == 'dir' || $this->items[$this->ordered_items[$index]]->get_type() == 'dokeos_chapter'
                ) {
                    return $this->index;
                } else {
                    return $index;
    ...
    public function get_next_item_id()
    {
        ...
        if (!empty ($new_index)) {
            if (isset ($this->ordered_items[$new_index])) {
                if ($this->debug > 2) {
                    error_log(
                        'New LP - In learnpath::get_next_index() - Returning '.$this->ordered_items[$new_index],
                        0
                    );
                }
    ...
    public static function get_package_type($file_path, $file_name)
    {
        ...
        if (!empty($_POST['ppt2lp']) && !in_array(
        ...
        if (!empty($_POST['woogie']) && !in_array(
            strtolower($extension),
            array(
                'dll',
                'exe'
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function get_previous_index()
    {
        ...
        if (isset ($this->ordered_items[$index - 1])) {
            $index--;
            while (isset ($this->ordered_items[$index]) AND ($this->items[$this->ordered_items[$index]]->get_type(
            ) == 'dir' || $this->items[$this->ordered_items[$index]]->get_type() == 'dokeos_chapter')) {
                $index--;
                if ($index < 0) {
                    return $this->index;
        ...
        if ($num > 0) {
            ...
            while ($row = Database :: fetch_array($res)) {
                $list[] = array(
                    ...
                    'id' => urldecode($row['interaction_id']),
                    //urldecode because they often have %2F or stuff like that
    ...
    public function get_iv_objectives_array($lp_iv_id = 0)
    {
        ...
        if ($num > 0) {
            ...
            while ($row = Database :: fetch_array($res)) {
                $list[] = array(
                    'order_id' => ($row['order_id'] + 1),
                    'objective_id' => urldecode($row['objective_id']),
                    // urldecode() because they often have %2F or stuff like that.
    ...
    public function get_html_toc($toc_list = null)
    {
        ...
        if ($is_allowed_to_edit && $hide_teacher_icons_lp == false) {
            ...
            if ($this->get_lp_session_id() == api_get_session_id()) {
                ...
                $html .= "<a class='btn' href='lp_controller.php?".api_get_cidreq(
                )."&amp;gradebook=$gradebook&amp;action=build&amp;lp_id=".$this->lp_id."' target='_parent'>".get_lang(
                    'Overview'
                )."</a>";
                $html .= "<a class='btn' href='lp_controller.php?".api_get_cidreq(
                )."&amp;action=add_item&amp;type=step&amp;lp_id=".$this->lp_id."' target='_parent'>".get_lang(
                    'Edit'
                )."</a>";
                $html .= '<a class="btn" href="lp_controller.php?'.api_get_cidreq(
                )."&amp;gradebook=$gradebook&amp;action=edit&amp;lp_id=".$this->lp_id.'">'.get_lang('Settings').'</a>';
                $html .= '</div>';
                $html .= '</div>';
            }
        }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function get_link($type = 'http', $item_id = null, $provided_toc = false)
    {
        ...
        if (empty($item_id)) {
            if ($this->debug > 2) {
                error_log(
                    'New LP - In learnpath::get_link() - no item id given in learnpath::get_link(), using current: '.$this->get_current_item_id(
                    ),
                    0
                );
            }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
                ...
                case 2 :
                    ...
                    if ($lp_item_type != 'dir') {
                        ...
                        if (preg_match('#^[a-zA-Z]{2,5}://#', $lp_item_path) != 0) {
                            if ($this->debug > 2) {
                                error_log(
                                    'New LP - In learnpath::get_link() '.__LINE__.' - Found match for protocol in '.$lp_item_path,
                                    0
                                );
                            }
                            ...
                            if ($this->debug > 2) {
                                error_log(
                                    'New LP - In learnpath::get_link() '.__LINE__.' - No starting protocol in '.$lp_item_path,
                                    0
                                );
                            }
                ...
                case 3 :
                    ...
                    $aicc_append = '?aicc_sid='.urlencode(session_id()).'&aicc_url='.urlencode(
                        api_get_path(WEB_CODE_PATH).'newscorm/aicc_hacp.php'
                    ).'&';
                    if ($lp_item_type != 'dir') {
                        // Quite complex here:
                        // We want to make sure 'http://' (and similar) links can
                        ...
                        if (preg_match('#^[a-zA-Z]{2,5}://#', $lp_item_path) != 0) {
                            ...
                            */
                            if (stripos($file, '<servername>') !== false) {
                                //$file = str_replace('<servername>',$course_path.'/scorm/'.$lp_path.'/',$lp_item_path);
                                $web_course_path = str_replace(
                                    'https://',
                                    '',
                                    str_replace('http://', '', $course_path)
                                );
...
<<<<<<< HEAD
        		WHERE c_id = ".$course_id." AND lp_id = ".$this->get_id()." AND user_id = ".$this->get_user_id(
        )." ".$search.
            " ORDER BY view_count DESC";
=======
        		WHERE
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function prerequisites_match($item = null)
    {
        ...
        if (isset($this->items[$item]) && is_object($this->items[$item])) {
            ...
            $result = $this->items[$item]->parse_prereq(
                $prereq_string,
                $this->items,
                $this->refs_list,
                $this->get_user_id()
...
<<<<<<< HEAD
    ...
    public function set_category_id($category_id)
    {
        ...
        $lp_table = Database :: get_course_table(TABLE_LP_MAIN);
        $lp_id = $this->get_id();
        $sql = "UPDATE $lp_table SET category_id = '".intval(
            $category_id
        )."' WHERE c_id = ".$course_id." AND id = '$lp_id'";
        Database::query($sql);
            ...
            switch ($this->get_type()) {
                case '3' :
                    if ($this->items[$this->last]->get_type() != 'au') {
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - In learnpath::stop_previous_item() - '.$this->last.' in lp_type 3 is <> au',
                                0
                            );
                        }
                        ...
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - In learnpath::stop_previous_item() - Item is an AU, saving is managed by AICC signals',
                                0
                            );
                        }
                    }
                case '2' :
                    if ($this->items[$this->last]->get_type() != 'sco') {
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - In learnpath::stop_previous_item() - '.$this->last.' in lp_type 2 is <> sco',
                                0
                            );
                        }
                        $this->items[$this->last]->close();
                        ...
                        if ($this->debug > 2) {
                            error_log(
                                'New LP - In learnpath::stop_previous_item() - Item is a SCO, saving is managed by SCO signals',
                                0
                            );
                        }
                    }
                ...
                default :
                    if ($this->debug > 2) {
                        error_log(
                            'New LP - In learnpath::stop_previous_item() - '.$this->last.' in lp_type 1 is asset',
                            0
                        );
                    }
                    $this->items[$this->last]->close();
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
            ...
            if ($this->debug > 2) {
                error_log(
                    'New LP - Problem in update_default_scorm_commit() - could not find LP '.$this->get_id().' in DB',
                    0
                );
            }
        }
...
<<<<<<< HEAD
    ...
    public function set_seriousgame_mode()
    {
        ...
        if (Database :: num_rows($res) > 0) {
            ...
            if ($this->debug > 2) {
                error_log(
                    'New LP - Problem in set_seriousgame_mode() - could not find LP '.$this->get_id().' in DB',
                    0
                );
            }
        }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if ($update_audio == 'true') {
            $return .= '<form action="'.api_get_self().'?cidReq='.Security :: remove_XSS(
                $_GET['cidReq']
            ).'&amp;updateaudio='.Security :: remove_XSS($_GET['updateaudio']).'&amp;action='.Security :: remove_XSS(
                $_GET['action']
...
<<<<<<< HEAD
        ...
        if ($update_audio != 'true') {
            ...
            $return .= '</div>';
            $return .= Display::div(
                Display::url(get_lang('Save'), '#', array('id' => 'listSubmit', 'class' => 'btn')),
        ...
        if ($update_audio == 'true') {
            $return .= '<div style="margin:40px 0; float:right;"><button class="save" type="submit" name="save_audio" id="save_audio">'.get_lang(
                'SaveAudioAndOrganization'
            ).'</button></div>'; // TODO: What kind of language variable is this?
        }
=======
	$return .= '<div id="message"></div>';
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        for ($i = 0; $i < count($arrLP); $i++) {
            ...
            if ($arrLP[$i]['item_type'] == 'document') {
                ...
                ).'&amp;action=view_item&amp;mode=preview_document&amp;id='.$arrLP[$i]['id'].'&amp;lp_id='.$this->lp_id;
            ...
            if (!$update_audio OR $update_audio <> 'true') {
                ...
                if ($arrLP[$i]['item_type'] != 'dokeos_chapter' && $arrLP[$i]['item_type'] != 'dokeos_module' && $arrLP[$i]['item_type'] != 'dir') {
                    ...
                    if (!empty ($arrLP[$i]['audio'])) {
<<<<<<< HEAD
                        $audio .= '<br />'.Security::remove_XSS(
                            $arrLP[$i]['audio']
                        ).'<br /><input type="checkbox" name="removemp3'.$arrLP[$i]['id'].'" id="checkbox'.$arrLP[$i]['id'].'" />'.get_lang(
                            'RemoveAudio'
                        );
=======
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
            ...
            if ($update_audio != 'true') {
                $row = $move_icon.' '.$icon.Display::span($title_cut).Display::span(
                    $audio.$edit_icon.$prerequisities_icon.$move_item_icon.$audio_icon.$delete_icon,
                    array('class' => 'button_actions')
                );
            } else {
    ...
    function print_recursive($elements, $default_data, $default_content)
    {
        ...
        foreach ($elements as $key => $item) {
            ...
            if (empty($item['children'])) {
                ...
                $sub_list = Display::tag(
                    'ul',
                    $sub_list.$data,
                    array('id' => 'UL_'.$key, 'class' => 'record li_container')
                );
                $return .= Display::tag(
                    'li',
...
<<<<<<< HEAD
    public function build_action_menu()
    {
        ...
        ).'&amp;gradebook='.$gradebook.'&action=view&lp_id='.$_SESSION['oLP']->lp_id.'&isStudentView=true">'.Display :: return_icon(
            'preview_view.png',
            get_lang('Display'),
            '',
            ICON_SIZE_MEDIUM
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        $buttons = array(
            array(
                ...
                ).'&amp;action=set_previous_step_as_prerequisite&amp;lp_id='.$_SESSION['oLP']->lp_id,
            ),
            array(
                'title' => get_lang('ClearAllPrerequisites'),
                'href' => 'lp_controller.php?'.api_get_cidreq(
                ).'&amp;action=clear_prerequisites&amp;lp_id='.$_SESSION['oLP']->lp_id,
            ),
        );
...
<<<<<<< HEAD
        ...
        if (!is_dir($filepath.'/'.$dir)) {
            $folder = FileManager::create_unexisting_directory(
                $course,
                api_get_user_id(),
                api_get_session_id(),
                0,
            ...
            if ($fp = @ fopen($filepath.$filename, 'w')) {
                ...
                if ($document_id) {
                    ...
                    if ($new_comment || $new_title) {
                        ...
                        if ($new_title) {
                            $ct .= ", title='".Database::escape_string(
                                htmlspecialchars($new_title, ENT_QUOTES, $charset)
                            )."' ";
                        }

                        $sql_update = "UPDATE ".$tbl_doc." SET ".substr(
                            $ct,
                            1
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
                ...
                $sql_update = "UPDATE ".$table_doc." SET title='".Database::escape_string(
                    $_POST['title']
                )."' WHERE c_id = ".$course_id." AND id = ".$document_id;
                Database::query($sql_update);
            }
        ...
        if (is_numeric($item_id)) {
            ...
            while ($row = Database :: fetch_array($result, 'ASSOC')) {
                ...
                switch ($row['item_type']) {
                    ...
                    case TOOL_DOCUMENT:
                        $tbl_doc = Database :: get_course_table(TABLE_DOCUMENT);
                        $sql_doc = "SELECT path FROM ".$tbl_doc." WHERE c_id = ".$course_id." AND id = ".Database::escape_string(
                            $row['path']
                        );
            ...
            switch ($row['item_type']) {
                ...
                case TOOL_LINK :
                    ...
                    if (ctype_digit($link_id)) {
                        ...
                        $sql_select = 'SELECT url FROM '.$tbl_link.' WHERE c_id = '.$course_id.' AND id = '.Database::escape_string(
                            $link_id
                        );
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function display_quiz_form($action = 'add', $id = 0, $extra_info = '')
    {
        ...
        if ($id != 0 && is_array($extra_info)) {
            ...
            $sql_quiz = "SELECT title, description FROM ".$tbl_quiz."
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        for ($i = 0; $i < count($arrLP); $i++) {
            if ($action != 'add') {
                if (($arrLP[$i]['item_type'] == 'dokeos_module' || $arrLP[$i]['item_type'] == 'dokeos_chapter' || $arrLP[$i]['item_type'] == 'dir') && !in_array(
                    $arrLP[$i]['id'],
                    $arrHide
...
<<<<<<< HEAD
                ...
                $return .= '<option '.$selected.'value="'.$arrLP[$i]['id'].'">'.get_lang(
                    'After'
                ).' "'.$arrLP[$i]['title'].'"</option>';
=======
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        $return .= '<tr>';
        if ($action == 'add') {
            $return .= '<td>&nbsp;</td><td><button class="save" name="submit_button" type="submit">'.get_lang(
                'AddExercise'
            ).'</button></td>';
        } else {
            $return .= '<td>&nbsp;</td><td><button class="save" name="submit_button" type="submit">'.get_lang(
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function display_hotpotatoes_form($action = 'add', $id = 0, $extra_info = '')
    {
        ...
        $return .= '<tr>';
        $return .= '<td>&nbsp; </td><td><button class="save" name="submit_button" action="edit" type="submit">'.get_lang(
            'SaveHotpotatoes'
        ).'</button></td>';
        $return .= '</tr>';
        $return .= '</table>';
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if ($action != 'move') {
            ...
            $return .= '<tr>';
            $return .= '<td class="label"><label for="idPrerequisites">'.get_lang(
                'LearnpathPrerequisites'
            ).'</label></td>';
            $return .= '<td class="input"><select name="prerequisites" id="prerequisites"><option value="0">'.get_lang(
                'NoPrerequisites'
            ).'</option>';
        ...
        $return .= '<tr>';
        $return .= '<td></td><td>
                    <button class="save" name="submit_button" type="submit" value="'.get_lang('Ok').'" />'.get_lang(
            'Ok'
        ).'</button></td>';
        $return .= '</tr>';
        $return .= '</table>';
    ...
    public function display_item_form($item_type, $title = '', $action = 'add_item', $id = 0, $extra_info = 'new')
    {
        ...
        if ($id != 0 && is_array($extra_info)) {
            $item_title = $extra_info['title'];
            $item_description = $extra_info['description'];
            $item_path = api_get_path(WEB_COURSE_PATH).$_course['path'].'/scorm/'.$this->path.'/'.stripslashes(
                $extra_info['path']
            );
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        $url = api_get_self().'?'.api_get_cidreq(
        ).'&gradeboook='.$gradebook.'&action='.$action.'&type='.$item_type.'&lp_id='.$this->lp_id;
        ...
        if ($item_type != 'module' && $item_type != 'dokeos_module') {
            ...
            if ($action != 'move') {
                $form->addElement(
                    ...
                    'title',
                    get_lang('Title'),
                    'id="idTitle" class="learnpath_chapter_form" size="40%"'
                );
            ...
            $parent_select = $form->addElement(
                ...
                get_lang('Parent'),
                '',
                'class="learnpath_chapter_form" style="width:37%;" id="idParent" onchange="javascript: load_cbo(this.value);"'
            );
...
<<<<<<< HEAD
        $position = $form->addElement(
            ...
            get_lang('Position'),
            '',
            'id="previous" class="learnpath_chapter_form" style="width:37%;"'
        );
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if (($item_type == 'sco') && ($extension == 'html' || $extension == 'htm')) {
            ...
            if ($item_type == 'sco') {
                $form->addElement(
                    'html',
                    '<script type="text/javascript">alert("'.get_lang('WarningWhenEditingScorm').'")</script>'
                );
            }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        $form = new FormValidator('form', 'POST', api_get_self(
        ).'?'.$_SERVER['QUERY_STRING'], '', array('enctype' => "multipart/form-data"));
        ...
        $parent_select = $form->addElement(
            ...
            get_lang('Parent'),
            '',
            'class="learnpath_item_form" id="idParent" style="width:40%;" onchange="javascript: load_cbo(this.value);"'
        );
        $my_count = 0;
        foreach ($arrHide as $key => $value) {
            if ($my_count != 0) {
        ...
        $position = $form->addElement(
            ...
            get_lang('Position'),
            '',
            'id="previous" class="learnpath_item_form" style="width:40%;"'
        );
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if (is_numeric($extra_info)) {
            $form->addElement(
                'style_submit_button',
                'submit_button',
                get_lang('SaveDocument'),
                'value="submit_button", class="save"'
            );
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if ($action != 'move') {
            ...
            $return .= '<td class="input"><input'.(is_numeric(
                $extra_info
            ) ? ' disabled="disabled"' : '').' id="idURL" name="url" style="width:99%;" type="text" value="'.$item_url.'" class="learnpath_item_form" /></td>';
            $return .= '</tr>';
            $id_prerequisite = 0;
            if (is_array($arrLP)) {
                foreach ($arrLP as $key => $value) {
    ...
    public function display_student_publication_form($action = 'add', $id = 0, $extra_info = '')
    {
        ...
        $return .= '<tr>';
        if ($action == 'add') {
            $return .= '<td>&nbsp</td><td><button class="save" name="submit_button" type="submit">'.get_lang(
...
<<<<<<< HEAD
        $url = api_get_self().'?cidReq='.Security::remove_XSS(
            $_GET['cidReq']
        ).'&view=build&id='.$item_id.'&lp_id='.$this->lp_id;
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function get_exercises()
    {
        ...
        while ($row_hot = Database :: fetch_array($res_hot)) {
            ...
            $return .= '<a href="'.api_get_self().'?'.api_get_cidreq(
            ).'&amp;action=add_item&amp;type='.TOOL_HOTPOTATOES.'&amp;file='.$row_hot['id'].'&amp;lp_id='.$this->lp_id.'">'.
                ((!empty ($row_hot['comment'])) ? $row_hot['comment'] : Security :: remove_XSS(
                    $row_hot['title']
                )).'</a>';
            $return .= '</li>';
        }
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function get_links()
    {
        ...
        $return .= '<a href="'.api_get_path(REL_CODE_PATH).'link/link.php?'.api_get_cidreq(
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
    ...
    public function get_student_publications()
    {
        ...
        $return .= Display::return_icon('works_small.gif');
...
>>>>>>> 671b81dac4dc97d884c25abdb2468903ec20cf84
        ...
        if (is_dir($current_course_path.'/scorm/'.$this->path) && is_file(
            $current_course_path.'/scorm/'.$this->path.'/imsmanifest.xml'
        )
        ) {
        ...
        foreach ($this->items as $index => $item) {
            if (!in_array($item->type, array(TOOL_QUIZ, TOOL_FORUM, TOOL_THREAD, TOOL_LINK, TOOL_STUDENTPUBLICATION))) {
                ...
                $my_title = $xmldoc->createElement(
                    'title',
                    htmlspecialchars(api_utf8_encode($item->get_title()), ENT_QUOTES, 'UTF-8')
                );
                    ...
                    if ($doc_info[1] == 'remote') {
                        ...
                        switch ($doc_info[2]) {
                            case 'url': // Local URL - save path as url for now, don't zip file.
                                $abs_path = api_get_path(SYS_PATH).str_replace(
                                ...
                                if (strstr($file_path, $main_path) !== false) {
                                    ...
                                    $link_updates[$my_file_path][] = array(
                                        'orig' => $doc_info[0],
                                        'dest' => $file_path
                                    );
                                    $my_dep_file->setAttribute('href', $file_path);
                            ...
                            case 'abs': // Absolute path from DocumentRoot. Save file and leave path as is in the zip.
                                ...
                                if ($pos === 0) {
                                    $abs_img_path_without_subdir = '/'.substr(
                                        $abs_img_path_without_subdir,
                                        strlen($relp)
                                    );
                                }
                                ...
                                $cur_path = substr(
                                    $current_course_path,
                                    -1
                                ) == '/' ? $current_course_path : $current_course_path.'/';
                                ...
                                if (strstr($file_path, $cur_path) !== false) {
                                    ...
                                    if (strstr($file_path, $cur_path) !== false) {
                                        ...
                                        $dotdot = '';
                                        $subdir = '';
                                        while (strstr($my_relative_path, $my_orig_file_path) === false && (strlen(
                                            $my_orig_file_path
                                        ) > 1) && (strlen($my_relative_path) > 1)) {
                ...
                switch ($item->type) {
                    case TOOL_LINK:
                        ...
                        $sql = 'SELECT url, title FROM '.Database :: get_course_table(
                            TABLE_LINK
                ...
                foreach ($list as $item_id) {
                    ...
                    switch ($item->type) {
                        case 'document':
                            ...
                            $file_path = api_get_path(
                                SYS_COURSE_PATH
                            ).$course_data['path'].'/document'.$file_data['path'];
                            if (file_exists($file_path)) {
